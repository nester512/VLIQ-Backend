-- DEMO data ONLY: sellers / receipts / payouts / transactions / promotions /
-- notifications for local development and the e2e test stand.
--
-- Applied by src/scripts/seed_dev.py ONLY when SEED_DEMO is truthy
-- (default: off) — a production restart must never re-create demo sellers or
-- unverifiable seed:// receipts in the admin review queue.
--
-- Idempotent: sellers are upserted by PK; dependent tables are cleared for the
-- demo seller IDs before being re-inserted.
--
-- Manual usage:
--   docker compose exec -T postgres psql -U vliq -d vliq < backend/seed_demo.sql

-- ---------------------------------------------------------------------------
-- Sellers — mix of pending / active / blocked across cities.
--   Cities are kept within the seed_dev.sql dictionary so editing a demo
--   seller in the app never trips the city validation.
