 ...
 *   3. Attempts to mint an admin JWT via POST /api/v1/auth/login { id: 99999 }.
 ...
 *      has telegram_id=99999 with role=super_admin but the JWT comes out as
...
async function globalSetup() {
  ...
  // table and returns role='seller'.  telegram_id=99999 exists in the admin
  ...
  try {
    ...
    if (adminRes.ok) {
      ...
      if (adminBody.role === 'admin' || adminBody.role === 'super_admin') {
        ...
        console.warn(
          `[globalSetup] SKIP admin tests — POST /auth/login { id: 99999 } returned role='${adminBody.role}' (expected 'admin' or 'super_admin'). ` +
